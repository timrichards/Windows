...
namespace DoubleFile
{
    partial class WinFileHashVM : ObservableObject_OwnerWindow
    {
        ...
        {
            mIcmd_OpenProject = new RelayCommand(param => { });
            mIcmd_SaveProject = new RelayCommand(param => { }, param => { return (m_lvVM.Count > 0); });

            mIcmd_NewListingFile = new RelayCommand(param => { });
            mIcmd_OpenListingFile = new RelayCommand(param => { });

            mIcmd_EditListingFile = new RelayCommand(param => { }, param => m_lvVM.SelectedOne());
            mIcmd_RemoveListingFile = new RelayCommand(param => { }, param => m_lvVM.SelectedAny());

            mIcmd_ToggleInclude = new RelayCommand(param => { }, param => m_lvVM.SelectedAny());
            mIcmd_VolumeGroup = new RelayCommand(param => { }, param => m_lvVM.SelectedAny());
        }

        internal void SetPartner(LV_FileHashVM lvVM)
        {
            m_lvVM = lvVM;
        }

        LV_FileHashVM m_lvVM = null;
    }
}
