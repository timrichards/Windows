    ...
    partial class WinVolumeList
    {
        void SaveDirListingsStatusCallback(string strPath, string strText = null, bool bDone = false, long nFilesTotal = 0, long nLengthTotal = 0, double nFilesDiff = 0)
        {
            ...
            if (nLengthTotal > 0)
            {
                ...
                strText = nFilesTotal.ToString("###,###,###,###") + " (" + FileUtil.FormatSize(nLengthTotal) + ") SD " + nFilesDiff.ToString("#0.0");
            }
            ...
            LVitem_VolumeVM lvItem = m_lvVM.ItemsCast.Where(i => (i.datum != null) /*&& (i.datum.ListView != null)*/).FirstOrDefault();
                ...
                lock (m_lvVM)
            {
                ...
                if (bDone && (strText == FileParse.mSTRsaved))
                {
                    ...
                    using (TextWriter fs = File.CreateText(strSaveAs))
                    {
                        ...
                        System.Threading.Timer timer = new System.Threading.Timer(new TimerCallback((Object state) =>
                        {
                            ...
                            if (m_listFileDiffs.Count >= 2)
                            {
                                ...
                                m_nFilesDiff = 0;
                                m_statusCallback(m_volStrings.Path, nFilesTotal: m_nFilesTotal, nLengthTotal: m_nLengthTotal, nFilesDiff: nFilesDiff);
                            }
